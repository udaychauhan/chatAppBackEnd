...
let forgotPassword = (req, res) => {
    ...
    let generateTokenToChangePassword = (userDetails) => {
        return new Promise((resolve, reject) => {
            token.generateTokenForPasswordChange(userDetails.email, (err, result) => {
                if (err) {
                    console.log(err)
                    ...
                    let apiResponse = response.generate(false, result.token, 200, null);
                    resolve(apiResponse)
                }
            });
        });
...
let verifyUserTokenAndChangePassword = (req, res) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (req.params.authToken || req.query.authToken || req.body.authToken || req.header('authToken')) {
                ...
                token.verifyClaimWithoutSecret(tokenFromRequest, (err, data) => {
                    if (err) {
                        let apiResponse = response.generate(false, "Token Error : " + err, 500, null)
                        reject(apiResponse);
                    } else {
                        resolve(data.data);
                    }
            ...
            if (req.params.password || req.query.password || req.body.password || req.header('password')) {
                ...
                UserModel.update({ 'email': email }, options).exec((err, result) => {
                    if (err) {
                        ...
                        let apiResponse = response.generate(false, "Password Changed For " + email, 200, null)
                        resolve(apiResponse);
                    }
                });// end user model update
...
module.exports = {
    ...
    verifyUserTokenAndChangePassword: verifyUserTokenAndChangePassword
